...
@Service
@Slf4j
public class NotificationAfterMessageSender {
    private final static Logger LOGGER = LoggerFactory.getLogger(NotificationAfterMessageSender.class);
...
//    @Transactional(propagation = Propagation.REQUIRES_NEW)
    protected void idleMessagesAsync() {
        LOGGER.info("Notification after consultation message service started");
        while (true) {
            try {
                log.info("Notification after consultation Slipping: " + calculateDifference());
                Thread.sleep(calculateDifference());
                check();
            } catch (InterruptedException ex) {
                log.info("Notification after consultation message service stopped: " + ex.getMessage());
            }
        }
    }
    ...
    protected void sendNotification(Client client, String message) {
        try {
            ...
            LOGGER.error("Unable to send notification after consultation to user: " + client.getTelegramId());
        }
    }

    @Async
    protected void check() {
        List<Client> clients = clientService.getByProcessedTrue();
